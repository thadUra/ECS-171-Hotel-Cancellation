...
7) [Collaboration Statement](#collaboration-statement)
8) [Prior Submissions](#prior-submissions)
...
### Second Model
For our second model, we decided to go for a SVM model. We chose this model over the other clustering models like Agglomerative or K-means, due to the fact that we only had two clusters in this case. We thought that we could run into a problem over-clustering as we had seen in HW 4 as Agglomerative had a tendency to cluster values into few clusters (this case 1), and K-means had a tendency to cluster values into good distributions (may not be what we are looking for). Even if we had more clusters, we would still threshold at halfway which would lead to similar results. Our last choices were between SVM and KNN. Thinking back, KNN may have been better and produced greater results with an adequate interval of neighbors, given that increasing its neighbors will not produce the same effects as adding more clusters. However, at the end of the day, we opted for SVM simply because of its widespread use for classification. In this case, we are classifying whether a reservation will be canceled. Overall, we were surprised that it happened to perform quite similarly to the ANN with 77% on the linear kernel and 75% on the RBF kernel. The repeated results somewhat makes us seem that it may be the data’s fault as to why its not achieving 100% accuracy, given two models seem to perform the same. As a result, this second model initiated the thought that a possible shortcoming for this project is the data itself.
